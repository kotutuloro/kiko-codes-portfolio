...
function Projects({ openModal, setModalContent }) {
    ...
    function showProjectModal() {
      const content = (
        <div className="flex flex-col items-center">
          <img
            className="rounded-xl max-h-[90vh] max-w-screen sm:max-w-[80vw]"
            src={image}
    ...
    return (
      <div className="bg-gradient-to-b from-purple-400 from-70% to-blue-600 p-px rounded-xl">
        <article
          ...
          className="hover:blur-sm cursor-pointer h-full transition ease-in duration-300 bg-neutral-900 rounded-xl overflow-hidden"
        >
          <img
          ...
          <div className="flex justify-between items-end gap-2 mx-2.5 mt-2 mb-1">
            <h2 className="font-bold text-xl text-left underline decoration-dotted decoration-purple-400">
              {title}
            </h2>
  ...
  return (
    <div>
      ...
      <div className="my-5 grid sm:grid-cols-2 gap-5">{projectElems}</div>
    </div>
  );
}
